...
public class HttpWebAPI
{
    ...
    public IActionResult Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequest req)
    {
        ...
        return new OkObjectResult("Welcome to Azure Functions! Lab 6 ");
    }
    ...
    public HttpResponseData GetStudentsBySchool(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "students/school/{school}")] HttpRequestData req, string school
    )
    {
        _logger.LogInformation($"Fetching students from the database for school: {school}");
        ...
        if (students.Length == 0)
        {
            ...
            responseNotFound.Headers.Add("Content-Type", "application/json");
            responseNotFound.WriteStringAsync($"No students found for school: {school}");
    ...
    public HttpResponseData GetStudentCountBySchool(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "students/count/count-by-school")] HttpRequestData req
    )
    {
        _logger.LogInformation("Fetching student count per school from database.");

        var schoolCounts = _context.Students
            .GroupBy(s => s.School)
            .Select(group => new { School = group.Key, Count = group.Count() })
            .OrderByDescending(s => s.Count)
            .ToArray();

        var response = req.CreateResponse(HttpStatusCode.OK);
        response.Headers.Add("Content-Type", "application/json");
        response.WriteStringAsync(JsonConvert.SerializeObject(schoolCounts));
